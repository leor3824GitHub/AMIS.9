...
namespace AMIS.WebApi.Inventories.Application.Inspections.Delete.v1
{
    public sealed class DeleteInspectionHandler(
        ...
        [FromKeyedServices("inventories:acceptances")] IReadRepository<Acceptance> acceptanceReadRepository)
        : IRequestHandler<DeleteInspectionCommand, DeleteInspectionResponse>
    {
        public async Task<DeleteInspectionResponse> Handle(DeleteInspectionCommand request, CancellationToken cancellationToken)
        {
            ...
            var hasAcceptance = await acceptanceReadRepository.AnyAsync(new AMIS.WebApi.Inventories.Application.Acceptances.Specifications.AcceptancesByInspectionIdSpec(request.Id), cancellationToken);
            if (hasAcceptance)
            {
