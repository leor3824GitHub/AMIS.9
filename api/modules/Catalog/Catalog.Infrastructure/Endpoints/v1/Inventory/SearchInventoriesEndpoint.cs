...
public static class SearchInventoriesEndpoint
{
    ...
    {
        return endpoints
            .MapPost("/search", async (ISender mediator, [FromBody] SearchInventoriesCommand command, ILoggerFactory loggerFactory) =>
            {
                var logger = loggerFactory.CreateLogger("SearchInventoriesEndpoint");
                logger.LogInformation(
                    "SearchInventories requested: PageNumber={PageNumber}, PageSize={PageSize}, HasKeyword={HasKeyword}, HasAdvancedSearch={HasAdvancedSearch}, HasAdvancedFilter={HasAdvancedFilter}",
                    command.PageNumber,
                    command.PageSize,
                ...
                logger.LogInformation(
                    "SearchInventories result: TotalCount={Total}, Returned={Returned}",
                    response.TotalCount,
