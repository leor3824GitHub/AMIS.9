...
public static class SearchAcceptanceItemsEndpoint
{
    ...
    {
        return endpoints
            .MapPost("/search", async (ISender mediator, [FromBody] SearchAcceptanceItemsCommand command, ILoggerFactory loggerFactory) =>
            {
                var logger = loggerFactory.CreateLogger("SearchAcceptanceItemsEndpoint");
                logger.LogInformation(
                    "SearchAcceptanceItems requested: PageNumber={PageNumber}, PageSize={PageSize}, HasKeyword={HasKeyword}, HasAdvancedSearch={HasAdvancedSearch}, HasAdvancedFilter={HasAdvancedFilter}",
                    command.PageNumber,
                    command.PageSize,
                ...
                logger.LogInformation(
                    "SearchAcceptanceItems result: TotalCount={Total}, Returned={Returned}",
                    response.TotalCount,
