...
public static class SearchPurchasesEndpoint
{
    ...
    {
        return endpoints
            .MapPost("/search", async (ISender mediator, [FromBody] SearchPurchasesCommand command, ILoggerFactory loggerFactory) =>
            {
                var logger = loggerFactory.CreateLogger("SearchPurchasesEndpoint");
                logger.LogInformation(
                    "SearchPurchases requested: PageNumber={PageNumber}, PageSize={PageSize}, HasKeyword={HasKeyword}, HasAdvancedSearch={HasAdvancedSearch}, HasAdvancedFilter={HasAdvancedFilter}",
                    command.PageNumber,
                    command.PageSize,
                ...
                logger.LogInformation(
                    "SearchPurchases result: TotalCount={Total}, Returned={Returned}",
                    response.TotalCount,
